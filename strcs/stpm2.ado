...
program Estimate, eclass byable(recall)
	...
	[, DF(string) TVC(varlist fv) DFTvc(string) KNOTS(numlist ascending) KNOTSTvc(string) ///
		...
		ALL RMAT REVerse CURE FAILCONVLININIT INITSTRATA(varlist) FROM(string)] ///
	[                               ///
...
/* knotstvc option */
/* add error if variable not specified */
	if "`knotstvc'" != "" {
		if "`dftvc'" != "" {
		...
		while "`2'"!="" {
			...
			if _rc == 0 {
				...
				display as error "You must give tvc variable names when using knotstvc() option."
				exit 198
			}
			cap confirm num `2'
			if _rc == 0 {
...
/* Generate splines for baseline hazard */
	...
	if "`df'" != "1" & "`rcsbaseoff'" == "" {
		...
		qui rcsgen `lnt' if `touse2', knots(`bhknots') gen(_rcs) dgen(_d_rcs) `orthog'  `rmatrixopt' `reverse'
		if "`orthog'" != "" {
			matrix `R_bh' = r(R)
		}
	}
	...
	if "`tvc'" != "" {
		foreach tvcvar in  `tvc' {
			if `tvc_`tvcvar'_df' != 1 {
				...
				qui rcsgen `lnt' if `touse2', knots(`tvcknots_`tvcvar'') gen(_rcs_`tvcvar') dgen(_d_rcs_`tvcvar') `orthog' `reverse'
				if "`orthog'" != "" {
					tempname R_`tvcvar' Rinv_`tvcvar'
					matrix `R_`tvcvar'' = r(R)
				}
...
/* Generate splines for delayed entry */
	if `del_entry' == 1 {
		...
		if "`df'" == "1" & "`rcsbaseoff'" == "" {
			qui rcsgen `lnt0' if `touse2' & _t0>0, gen(_s0_rcs) `reverse'
		}
		else if "`df'" != "1" & "`rcsbaseoff'" == "" {
			qui rcsgen `lnt0' if `touse2' & _t0>0, knots(`bhknots') gen(_s0_rcs) `reverse'
		}

		foreach tvcvar in  `tvc' {
			if `tvc_`tvcvar'_df' == 1 {
				qui rcsgen `lnt0' if `touse2' & _t0>0,  gen(_s0_rcs_`tvcvar') `reverse'
			}
			else if `tvc_`tvcvar'_df' != 1 {
				qui rcsgen `lnt0' if `touse2' & _t0>0, knots(`tvcknots_`tvcvar'') gen(_s0_rcs_`tvcvar') `reverse'
			}
		}		
	}
	...
	if "`orthog'" != "" {
		...
		if `del_entry' == 1 {
			...
			if "`rcsbaseoff'" == "" {
				...
				mata st_store(.,tokens(st_local("s0_rcsterms_base")),"`touse_t0'",(st_data(.,(tokens(st_local("s0_rcsterms_base")),"`cons'"),"`touse_t0'")*st_matrix("`Rinv_bh'"))[,1..`df'])	
			}
			if "`tvc'" != "" {
				foreach tvcvar in  `tvc' {
					matrix `Rinv_`tvcvar'' = inv(`R_`tvcvar'')
					mata st_store(.,tokens(st_local("s0_rcsterms_`tvcvar'")),"`touse_t0'",(st_data(.,(tokens(st_local("s0_rcsterms_`tvcvar'")),"`cons'"),"`touse_t0'")*st_matrix("`Rinv_`tvcvar''"))[,1..`tvc_`tvcvar'_df'])
				}
			}
		}
	}
...
/* Fit linear term to log(time) for initial values. */
		...
		if inlist("`scale'","hazard","odds","normal") {
			...
			if `del_entry' == 1 {
				local xb0 `"(xb0: `varlist' _s0_rcs1"' 
				if "`tvc'" != "" {
					foreach tvcvar in `tvc' {
			...
			qui ml model e2 stpm2_ml_`scale'`rs' ///
